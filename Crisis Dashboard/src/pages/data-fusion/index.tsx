...
const INITIAL_SATS: SatelliteHealth[] = [
  { name: "DSCOVR", health: "nominal", signal: "Strong", latency: 16, missingPercent: 0, trustScore: 85, contributionPercent: 25 },
  { name: "ACE", health: "nominal", signal: "Strong", latency: 21, missingPercent: 0, trustScore: 85, contributionPercent: 25 },
  { name: "WIND", health: "nominal", signal: "Strong", latency: 27, missingPercent: 0, trustScore: 85, contributionPercent: 25 },
  { name: "SOHO", health: "nominal", signal: "Strong", latency: 38, missingPercent: 0, trustScore: 95, contributionPercent: 25 }
];
...
export default function DataFusionPage() {
  const [sats, setSats] = useState<SatelliteHealth[]>(INITIAL_SATS);
  const [fusion, setFusion] = useState<FusionResult[]>(INITIAL_FUSION);
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        if (sData && sData.length > 0) setSats(sData);
        if (fData && fData.length > 0) setFusion(fData);
      } catch (err) {
        console.error(err);
      } finally {
        setSyncing(false);
